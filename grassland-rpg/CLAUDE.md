...
src/
  ...
  entities/
    ...
    Monster.js
    MonsterModel.js    # 몬스터 모양 고르기 + 재질별로 조각 합치기(bake, 그리기 호출 줄이기)
    monsterKit.js      # 몬스터 공통 디자인 키트 (차분한 색 tone·눈·눈썹·입·볼) + 밤 몬스터 모습(nightLook)
    monsterShapes.js   # 필드 몬스터 모양 (슬라임·버섯·선인장·골렘·벌·토끼·포자버섯·늑대·그루터기·전갈·두더지·굴렁덤불·요정·설인·눈사람)
    bossShapes.js      # 보스 모양 (왕슬라임·고목 수호자·밤의 군주)
    ...
    FxPool.js          # 짧은 빛 연출 풀 (바닥 고리·반짝 별)
  ...
  ui/
    ...
    Tooltip.js
    icons.js           # itemIcon(def) — 모든 창이 쓰는 아이템 아이콘
    itemArt.js         # 아이템 아이콘 아트 (48격자 SVG, 재질 팔레트·외곽선·명암·등급 뒷빛)
    uiArt.js           # 스킬 배지·건설(포탑·부속 건물·벽) 아이콘 (itemArt 와 같은 붓)
    styles.css         # 기본 HUD
    windows.css        # 창
...
### 4-14. 비주얼 스타일 (비주얼 품질 개선)
게임 수치·시스템은 그대로 두고 보이는 품질만 다룬다. 새 아이템·몬스터·아이콘도 이 규칙을 따른다.
- **팔레트**: 원색·형광색을 쓰지 않는다. 재질 팔레트(`itemArt.js`의 `M`: 나무·짙은 나무·대나무·끈·가죽·돌·모래·강철·쇠·금·얼음·젤리·이끼…)에서 고른다. 데이터의 `color`는 아이콘·옷에는 쓰지 않고, 몬스터는 `tone()`으로 채도를 낮춰 쓴다
- **아이콘**: 48격자 SVG. 따뜻한 짙은 갈색 외곽선 하나, 왼쪽 위 밝음 → 오른쪽 아래 어두움 그라디언트, 흰 하이라이트, 바닥 그림자. 실루엣만 봐도 무엇인지 알 수 있게 (검 = 날·가드·손잡이·폼멜, 활 = 활대·시위·손잡이)
- **등급**: 일반·고급은 칸 테두리만, 희귀 이상은 아이콘 뒤 등급 색 뒷빛, 전설만 가는 빛살. 바닥 드롭도 희귀 이상은 등급 색 빛 고리. 등급 색은 `items.json` `grades` (차분한 톤)
- **무기 3D**: 아이템마다 재질·장식이 다르다 (나뭇가지 검 = 깎은 가지+끈, 무쇠 = 강철 날+쇠 가드, 상위 = 금·보석·빛나는 룬). 바닥에 떨어진 무기도 같은 모양
- **장비 세트**: 초원 = 잎 삿갓·잎 깃·풀 허리끈 / 숲 = 버섯 모자·가죽 허리띠·장화 / 사막 = 두건·등 망토·샌들 끈 / 설원 = 털모자·털 칼라·털 부츠
- **몬스터**: 공통 얼굴(세로로 긴 눈+하이라이트, 사나운 종은 눈썹), 두 톤 몸(배·주둥이 밝게), 지역이 멀수록 장식이 늘어난다(초원 말랑 → 숲 버섯·이끼 → 사막 가시·거친 결 → 설원 얼음·털). 보스는 크고 고유 장식(왕관·잎 뭉치·달뿔). 조각은 재질별로 합친다
- **밤 몬스터**(밤에 태어난 필드 몬스터·습격 몬스터): 몸 전체를 보라로 칠하지 않는다. 몸은 살짝 어둡고 차갑게, 눈만 은은히 빛나고, 작은 빛 조각 둘이 맴돈다
- **밤 조명**: 밝기만 낮추지 않고 색을 바꾼다. 반구광 남색 하늘·쪽빛 땅, 차가운 달빛(그림자 유지), 플레이어 곁 따뜻한 등불, 모닥불·텐트의 따뜻한 빛. 붉은 달은 남색 밤 + 붉은 달빛 (화면 전체를 붉게 덮지 않는다)
...
- **모바일 배치**(`ui/mobile.css`, polish 다음에 읽음, `body.touch` 전용): 세로(≤600px)·아주 좁음(≤370px)·가로(높이 ≤500px) 세 경우로 HUD·☰·알림·창 크기를 정한다. 스킬 포인트는 `✦ +N` 만(`.sp-long` 숨김), 창은 화면 폭에 맞춘 세로 열 + 본문 스크롤, 스킬 창은 갈래 2열(가로 3열) + 아래 고정 확인 바. `.window[hidden]` 은 반드시 `display:none` 을 다시 적어 둔다 (flex 가 hidden 을 덮는다).

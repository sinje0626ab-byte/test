...
import { FxPool } from '../entities/FxPool.js';
import { tone } from '../entities/monsterKit.js';
...
export class FeedbackSystem {
  constructor(ctx, camera) {
    ...
    this.fx = new FxPool(ctx.scene, ctx.camera);
    const soft = (c) => tone(c ?? '#ffffff'); // 몬스터 모델과 같은 차분한 색
    this.hitstop = 0;
    this.shakeOn = true;
    this.dustTimer = 0;
    this.pillars = [];
    ...
    bus.on('combat:hit', (e) => {
      if (e.target === 'monster') {
        const at = e.position.clone().setY(0.7);
        this.particles.burst(at, { color: [soft(e.color), '#fff4dc'], count: P.hit, speed: 3.2, up: 3, life: 0.45 });
        this.fx.spark(at, e.crit ? { color: '#ffd27a', size: 1.0, life: 0.22 } : { size: 0.55 });
        if (e.crit) this.fx.ring(e.position, { color: '#ffd27a', to: 1.6, life: 0.3 });
        if (e.source === 'player') this.stop(e.crit ? H.crit : H.hit);
        if (e.crit) this.shake(S.crit);
      } else if (e.target === 'player') {
        this.shake(S.playerHit);
        this.particles.burst(e.position.clone().setY(0.8), { color: ['#e0706a', '#ffffff'], count: 5, speed: 2.5, up: 2.5, life: 0.4 });
        this.fx.spark(e.position.clone().setY(0.9), { color: '#ff8a7a', size: 0.7, life: 0.18 });
    ...
    bus.on('projectile:explode', (e) => {
      ...
      this.particles.burst(e.position.clone().setY(0.3), { color: ['#ffb35c', '#e0703a', '#5a5147'], count: P.blast, speed: 5, up: 5, life: 0.6, size: 0.12 });
      const frost = e.effect?.type === 'slow';
      this.fx.ring(e.position, { color: frost ? '#9fdcff' : '#ffb35c', to: e.radius ?? 2.2, life: 0.4 });
      this.fx.spark(e.position.clone().setY(0.6), { color: frost ? '#dff6ff' : '#ffd08a', size: 1.2, life: 0.18 });
    });
    bus.on('boss:aoe', (e) => {
      ...
      this.fx.ring(e.position, { color: '#ffe2b0', from: e.radius * 0.4, to: e.radius, life: 0.4 });
    });
    // 보스 등장: 발밑에서 큰 고리 두 겹
    bus.on('boss:engaged', ({ boss }) => {
      this.shake(0.2);
      this.fx.ring(boss.position, { color: soft(boss.def.color), from: 0.5, to: boss.radius * 4, life: 0.8, opacity: 0.8 });
      this.fx.ring(boss.position, { color: '#ffffff', from: 0.3, to: boss.radius * 2.5, life: 0.6, opacity: 0.5 });
    });
    // 몬스터 다양화 (Phase 10)
    bus.on('monster:blast', (e) => {
      const d = e.position.distanceTo(ctx.player.position);
      this.shake(S.cannon * Math.max(0, 1 - d / S.cannonFalloff));
      this.particles.burst(e.position.clone().setY(0.4), { color: ['#ffb35c', '#e0703a', soft(e.monster.def.color)], count: P.blast, speed: e.radius * 2.5, up: 5, life: 0.6, size: 0.13 });
      this.fx.ring(e.position, { color: '#ffb35c', to: e.radius, life: 0.4 });
    });
    ...
    bus.on('boss:split', ({ boss }) => {
      this.shake(S.bossAoe);
      this.particles.burst(boss.position.clone().setY(1), { color: soft(boss.def.color), count: P.blast, speed: 6, up: 5, life: 0.7, size: 0.16 });
      this.fx.ring(boss.position, { color: soft(boss.def.color), to: boss.radius * 3, life: 0.6 });
    });
    // 액티브 스킬 (Phase 11)

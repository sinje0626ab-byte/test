...
export class BuildSystem {
  ...
  validate(pos) {
    ...
    if (count >= base.maxTurrets) return { ok: false, reason: `이 기지엔 포탑을 더 세울 수 없습니다 (${count}/${base.maxTurrets})` };
  ...
  update() {
    ...
    if (p.kind === 'turret') {
      const spend = { amount: this.ctx.data.turrets[p.type].cost, ok: false };

...
export class BuildMenu {
  ...
  render() {
    ...
    this.infoEl.textContent = `${base.name} Lv${base.level} · 포탑 ${count}/${base.maxTurrets}`;
    ...
    const cards = Object.entries(ctx.data.turrets).map(([id, t]) => {
      ...
      const full = count >= base.maxTurrets;
      const poor = this.gold < t.cost;
      ...
      return `
        <button type="button" class="build-card${locked || full || poor ? ' disabled' : ''}" data-turret="${id}">
          ...
          <span class="build-text">
            ...
            <small class="stats">공격 ${t.damage} · 사거리 ${t.range} · 초당 ${t.fireRate}발 · 체력 ${t.hp}</small>
          </span>
          <span class="build-cost"><i class="coin"></i>${t.cost}<small>${why}</small></span>
        </button>`;
    });
